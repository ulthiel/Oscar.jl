...
#
# Apr 2023: Imported into OSCAR by UT.
################################################################################

export Composition, CompositionsFixedNumParts, Compositions, num_compositions, compositions
...
A **composition** (also called **ordered partition**) of an integer ``n ≥ 0`` is a sequence
``(λ₁,…,λₖ)`` of positive integers ``λᵢ`` whose sum is equal to ``n``. The ``λᵢ`` are called
the **parts** of the composition. 

We provide the parametric type `Composition{T}` which is a subtype of `AbstractVector{T}`
where `T` can be any subtype of `IntegerUnion`. The behavior and functionality is similar to
[`Partition`](@ref).

# Examples
```jldoctest
julia> C = Composition(3,4,3)
[3, 4, 3]
...
function Base.getindex(P::Composition, i::IntegerUnion)
    return getindex(P.p,i)
end

function Base.setindex!(P::Composition, x::IntegerUnion, i::IntegerUnion)
  return setindex!(P.p,x,i)
end

function Base.copy(P::Composition{T}) where T<:IntegerUnion
  return Composition{T}(copy(P.p))
end

function Composition(parts::IntegerUnion...)
  return Composition(collect(Int, parts))
end
  
function Composition{T}(parts::IntegerUnion...) where T<:IntegerUnion
  return Composition(collect(T, parts))
end

function Composition(p::Vector{Any})
  return Composition(Vector{Int}(p))
end
...
################################################################################
@doc raw"""
    num_compositions(n::IntegerUnion)
...
# References
1. [Bon17], Corollary 5.4
2. [OEIS](@cite), [A011782](https://oeis.org/A011782)
"""
function num_compositions(n::IntegerUnion)
...
################################################################################
@doc raw"""
    num_compositions(n::IntegerUnion, k::IntegerUnion)
...
"""
function num_compositions(n::IntegerUnion, k::IntegerUnion)
...
Returns an iterator over all compositions of n into k parts. The algorithm used is Algorithm
72 "Composition Generator" by Hellerman & Ogden (1961), which refers to Chapter 6 of Riordan
(1958). De-gotoed by E. Thiel.
...
# References
1. Hellerman, L. & Ogden, S. (1961). Algorithm 72: composition generator. *Communications of
   the ACM, 4*(11), 498.
   [https://doi.org/10.1145/366813.366837](https://doi.org/10.1145/366813.366837)
2. Riordan, J. (1958). *An introduction to Combinatorial Analysis*. John Wiley & Sons, Inc.,
   New York.
"""
struct CompositionsFixedNumParts{T<:IntegerUnion}
  n::T
  ...
  function CompositionsFixedNumParts(n::T,k::T) where T <: IntegerUnion
...
function Base.eltype(P::CompositionsFixedNumParts{T}) where T <: IntegerUnion
...
@inline function Base.iterate(P::CompositionsFixedNumParts{T}) where T <: IntegerUnion
...
"""
    compositons(n::IntegerUnion, k::IntegerUnion)
...
An iterator over all compositions of an integer ``n``. 

# Algorithm

The iterator iterates over compositions of ``n`` into ``k`` parts for ``1 ≤ k ≤ n`` using
the [`CompositionsFixedNumParts`](@ref) iterator.

# Examples
```jldoctest
julia> I = Compositions(4)
Iterator over compositions of 4
...
function Base.eltype(P::Compositions{T}) where T <: IntegerUnion
  return Composition{T}
end
...
# Iterator initialization
@inline function Base.iterate(P::Compositions{T}) where T <: IntegerUnion
...
# Actual algorithm
@inline function Base.iterate(P::Compositions{T}, state) where T <: IntegerUnion
...
# Algorithm
This uses the [`Compositions`](@ref) iterator.
"""
