...
Multipartitions are generalizations of partitions: an r-component **multipartition** of an
integer ``n`` is an ``r``-tuple of partitions ``λ¹, λ², …, λʳ`` where each ``λⁱ`` is a
partition of some integer ``nᵢ ≥ 0`` and the ``nᵢ`` sum to ``n``.

We provide the parametric type `Multipartition{T}` which is a subtype of
`AbstractVector{Partition{T}}` where `T` can be any subtype of `IntegerUnion`. See
[`Partition{T}`](@ref) for the basic ideas, which equally apply here.
